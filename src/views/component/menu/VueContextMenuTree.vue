        ...
        <li v-for="i in itemchildren" :key="i.name" class="child-li-wrapper">
            <div v-if="i.children && i.children.length > 0" class="has-child">
                <div class="parent-name btn-wrapper-simple">
                    ...
                    <i class="icon"></i></div>
                <VueContextMenuTree :itemchildren="i.children" @childhandler="fnHandler" :float="float"/>
            </div>
            <div v-else>
...
<script lang="ts">
    ...
    export default class VueContextMenuTree extends Vue {
        @Prop({
            type: Array,
            required: false,
            default: () => ([]),
        ...
        private fnHandler(item: any): void {
